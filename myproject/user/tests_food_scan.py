...
class TestFoodScanServiceLogic(unittest.TestCase):
    ...
    def test_json_extraction_markdown(self):
        ...
        mock_response.text = '```json\n{"items": [{"name": "Apple", "confidence": 0.9, "estimated_per_100g": {"calories": 52, "fiber": 2.4}, "pro_tip": "Keep the skin for fiber!"}]}\n```'
        ...
        self.assertEqual(items[0]['fiber'], 2.4)
        self.assertEqual(items[0]['pro_tip'], 'Keep the skin for fiber!')
    ...
    def test_edamam_fallback(self, mock_requests_get):
        ...
        mock_edamam_response.json.return_value = {
            "hints": [{
                "food": {
                    "label": "Blueberry",
                    "nutrients": {"ENERC_KCAL": 57, "PROCNT": 0.7, "CHOCDF": 14, "FAT": 0.3, "FIBTG": 2.4}
                }
            }]
        }
        ...
        self.assertEqual(items[0]['fiber'], 2.4) # From Edamam (FIBTG)
        self.assertEqual(items[0]['name'], "Blueberry")

    def test_full_nutritional_response(self):
        mock_response = MagicMock()
        mock_response.text = json.dumps({
            "items": [{
                "name": "Pizza",
                ...
                "healthier_alternative": "Try cauliflower crust",
                "pro_tip": "Add veggies to lower GI"
            }]
        })
        self.service.model.generate_content.return_value = mock_response
        mock_models.FoodItem.objects.filter.return_value.first.return_value = None
        ...
        self.assertEqual(len(items), 1)
        item = items[0]
        self.assertEqual(item['calories'], 266)
        self.assertEqual(item['protein'], 11)
        self.assertEqual(item['vitaminC'], 5)
        self.assertEqual(item['calcium'], 200)
        self.assertEqual(item['healthier_alternative'], "Try cauliflower crust")
        self.assertEqual(item['pro_tip'], "Add veggies to lower GI")
